 ...
 * asset URL became `/VetCrewEmergency/…`, all 60 of them 404'd at the root, and
 ...
 */
const PAGES = [
	// Головна. Перше, що відкривають, і єдине, що має значення о третій ночі.
	'/',
	// Найважча сторінка збірки: увесь перелік бібліотеки з десятьма картками.
	// Якщо бюджет десь ламається, він ламається тут першим.
	'/library',
	// Сторінка статті — те, що читають у полі, з телефона й на поганому звʼязку.
	'/library/fawn',
	// Єдина сторінка, чий вміст береться зі сховища, а не зі збірки.
	'/saved',
	// Локалізована головна: разом із нею перевіряються `<html lang>` і набір
	// hreflang, а не лише українське дерево.
	'/en',
...
module.exports = {
	ci: {
		collect: {
			...
			// vite prints `➜  Local:   http://localhost:4174/VetCrewEmergency` when it is up.
