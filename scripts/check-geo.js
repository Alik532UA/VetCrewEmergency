...
export function checkGeo(buildDir, options = {}) {
	 ...
	 * просто не написали.
	 *
	 * **Приховані сторінки теж**, і виняток для них тут був помилкою. Міркування
	 * «`noindex`, отже опису ніхто не прочитає» правильне лише про пошуковик:
	 * опис показує перегляд посилання, а Lighthouse рахує його відсутність
	 * окремим провальним аудитом. Ціна виміряна: на прихованій сторінці це
	 * різниця між 0.63 і 0.54 (lighthouserc.cjs), тобто виняток коштував
	 * калібрований поріг у CI — заради двох ненаписаних рядків.
	 *
	 * Єдиний виняток лишається справжнім: оболонка SPA описувати нічого не може,
	 * бо в неї порожнє тіло за побудовою.
	 */
	...
	if (!existsSync(llmsPath)) {
		...
		if (spaFallback) {
			 ...
			 * стояв хост `VetCrewEmergency.github.io` замість `alik532ua.github.io`,
