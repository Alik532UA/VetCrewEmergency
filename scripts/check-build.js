...
/** `beta-test-checklists` and `en/beta-test-checklists` are the same hidden route. */
...
// named `https://alik532ua.github.io/VetCrewEmergency` as its own canonical, its
...
/*
 * Порожній: двох сторінок з однаковим описом у цій збірці немає. Запис із
 * двома тваринами на імʼя BLACK пішов разом із розділом прихистку.
 *
 * Перелік лишається, бо однаковий опис — це не завжди дефект: буває, що
 * дві сторінки й справді про те саме. Тоді рішення записується сюди, а не
 * лишається мовчазним фактом.
 */
...
/*
 * Мови й типова — копія з `src/lib/i18n/locales.ts`. Копія неминуча: скрипт
 * ходить по `build/` звичайним Node і модулів застосунку не імпортує. Але
 * копія, яка мовчки застаріває, і є той дефект, проти якого написані решта
 * перевірок цього файлу, — тож розходження ловить `src/docs-numbers.test.ts`.
 */
const LOCALES = ['uk', 'en'];
...
// false of `/VetCrewEmergency/images/x`, which is the base spelled out.
//
...
// /VetCrewEmergency/adopt/images/logo/… on /adopt/cat/cucumber — 27 broken images in
