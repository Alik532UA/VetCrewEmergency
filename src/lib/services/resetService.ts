 ...
 * `vetcrewemergency_` prefix, which is exactly why the reset goes through it rather than
 ...
 * (`https://host/VetCrewEmergency/`) while `base` is a path (`/VetCrewEmergency`), so a direct `startsWith`
 * would never match.
 */
 ...
 * not delete the entry but duplicates it. `base` IS that path: `/VetCrewEmergency` in
