...
import { localeUrl, absoluteLocale, SITE_ORIGIN, SITE_BASE } from './config';
...
describe('absolute page addresses', () => {
	 ...
	 * GitHub Pages is no exception: `https://alik532ua.github.io/VetCrewEmergency`
	 * redirects to `…/VetCrewEmergency/`. Naming the redirecting form in a canonical
	...
	it('an inner page carries no trailing slash in any language', () => {
		for (const locale of LOCALES) {
			...
			expect(prefixedUrl('/library', locale)).toBe(
				`${SITE_ORIGIN}${PROJECT_SITE_BASE}${segment}/library`
			);
		}
	});
	...
	it('absoluteLocale() is localeUrl() with the build constants', () => {
		for (const locale of LOCALES) {
			expect(absoluteLocale('/library', locale)).toBe(
				localeUrl(SITE_ORIGIN, SITE_BASE, '/library', locale)
			);
		}
	});
});
