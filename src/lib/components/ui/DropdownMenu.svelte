<script lang="ts">
	...
	/** Відступ меню від тригера — те саме число, що в `top: calc(100% + 8px)` нижче. */
	const GAP = 8;

	/**
	 * Менше за це меню не стискаємо, навіть якщо місця під тригером зовсім немає.
	 *
	 * Ця підлога тут як запобіжник, а не як робоче значення: тригер живе в шапці,
	 * тобто вгорі вікна, і місця під ним завжди вистачає. Якби колись меню
	 * відкривали знизу екрана, краще показати смужку, якою можна прокрутити, ніж
	 * коробку у два пікселі.
	 */
	...
	function place() {
		...
		/*
		 * Стеля висоти — вертикальний відповідник `max-width` у стилях.
		 *
		 * Ширину меню міряли й обмежували від початку, а висоту ні: поки груп було
		 * три, меню вміщалося в будь-яке вікно. Із п'ятьма на низькому екрані воно
		 * впиралося в нижній край, і останні пункти ставали недосяжні — не приховані,
		 * а саме недосяжні, бо прокручувати не було чого.
		 *
		 * Міряється так само, як бік: скільки місця лишилося під тригером. Числом
		 * писати не можна — висота шапки в цьому проєкті одна, але тригер вимірюється
		 * там, де він фактично опинився.
		 */
		const below = window.innerHeight - anchor.bottom - GAP - EDGE;
		menuElement.style.setProperty('--menu-max-height', `${Math.max(below, MIN_HEIGHT)}px`);
	}
...
<style>
	...
	.dropdown__menu {
		...
		/*
		 * Типове значення на випадок першого кадру — до того, як `place()` зміряє
		 * справжнє місце під тригером.
		 *
		 * Оголошене окремим рядком, а не запасним значенням другим аргументом
		 * `var()`: таке запасне приховало б той день, коли вимірювання перестане
		 * відбуватися взагалі. Цього ж вимагає й гейт `css-variables.test.ts` — і
		 * ловить він не лише код: перша редакція цього коментаря НАЗВАЛА заборонений
		 * запис як приклад, і гейт почервонів на власному поясненні.
		 */
		--menu-max-height: calc(100vh - 96px);
		max-height: var(--menu-max-height);
		overflow-y: auto;
		/* Докрутивши меню до кінця, далі гортати починала б СТОРІНКА під ним. */
