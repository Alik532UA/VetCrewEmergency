...
<style>
	 ...
	 */
	/*
	 * Не в ряду шапки, а над сторінкою — як кнопка «нагору», тільки згори.
	 *
	 * Поки ряд стояв у потоці, його поява й зникання РОЗСУВАЛИ смугу: логотип,
	 * пункти меню й кнопки гарячої лінії щоразу переїжджали вбік. А службовий жест
	 * (сім натисків `H`) тим і незручний, що його вмикають і вимикають часто — тобто
	 * саме та дія, яка не мусить нічого рухати, рухала все.
	 *
	 * `position: fixed` під смугою, біля правого краю: панель тепер нічия сусідка.
	 * Те саме число висоти шапки, що в `HeaderNav.svelte` і `base.css`.
	 */
	.header__controls {
		position: fixed;
		z-index: 1400; /* Під смугою (1500), над вмістом сторінки. */
		top: calc(72px + var(--space-xs));
		right: var(--space-xl);
		display: none;
		align-items: center;
		gap: var(--space-xs);
