...
<style>
	...
	/*
	 * Дрібний рядок — без прозорості.
	 *
	 * `opacity: 0.85` тут стояв, поки заливка кнопки була суцільною: тоді білий із
	 * прозорістю лежав на відомому кольорі й лишався читомим. Відколи заливка стала
	 * напівпрозорою (`--btn-fill`), під написом видно сторінку, і в СВІТЛІЙ темі
	 * композит просів до 4.28:1 при потрібних 4.5 — axe знайшов це на `/support`,
	 * `/stories/…` і чеклисті. Дефект мій, і прозорість тут була його причиною.
	 *
	 * Ієрархія від цього не зникла: рядок і так на чверть дрібніший за номер, а в
	 * компактному варіанті ще й набраний капітеллю з розрядкою.
	 */
	.hotline__text small {
		font-size: 0.75em;
	}
