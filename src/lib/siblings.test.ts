...
describe('the sibling registry describes this site correctly', () => {
	const own = SIBLINGS.VetCrewEmergency;
	...
	it('carries the address the deploy workflow builds for', () => {
		...
		/*
		 * Межа рядка обов'язкова. З `\s*` порожнє значення читалося ДАЛІ, і
		 * регулярка захоплювала перше слово наступного рядка — `run:`. Тобто
		 * «власного домену немає» перетворювалося на домен `run:`, і гейт
		 * вимагав від реєстру адресу `https://run:`. Заміряно рівно тут, коли
		 * сайт переїхав на проєктну адресу GitHub Pages.
		 */
		const domain = /^[ \t]+CUSTOM_DOMAIN:[ \t]*(\S*)[ \t]*$/m.exec(workflow)?.[1] ?? '';

		if (domain) {
			expect(own.origin).toBe(`https://${domain}`);
			expect(own.base).toBe('');
		} else {
			expect(own.base.toLowerCase()).toBe(
				`/${JSON.parse(readFileSync('package.json', 'utf-8')).name}`.toLowerCase()
			);
		}
	});
