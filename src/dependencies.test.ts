...
/**
 * Dependabot існує й розділяє мажори (DEPENDENCIES-v9 § 3, `DEP-DEPENDABOT`).
 *
 * Правило перевіряється «наявністю `.github/dependabot.yml`» — тобто нічим, поки цього
 * не спитати. Файл, який ніхто не запускає й ніщо не стереже, зникає при першому
 * прибиранні теки, і зникає тихо: PR-ів просто перестає бути, а відсутність PR-а
 * виглядає точнісінько як «оновлень немає».
 *
 * Групи перевіряються разом із наявністю, бо саме вони й роблять налаштування корисним:
 * без `update-types: [minor, patch]` мажор приїжджає в одному PR із патчами, і зламану
 * зміну знаходять серед десятка нешкідливих.
 */
describe('Dependabot (§ 3)', () => {
	const CONFIG = '.github/dependabot.yml';

	it('конфіг на місці', () => {
		expect(existsSync(CONFIG), `${CONFIG} зник — оновлення перестануть приходити мовчки`).toBe(
			true
		);
	});

	it('патчі й мінори згруповані, тобто мажори лишаються окремими PR-ами', () => {
		const config = readFileSync(CONFIG, 'utf8');
		expect(config, 'екосистема npm не описана').toContain('package-ecosystem: npm');
		expect(config, 'групування зникло — мажор приїде разом із патчами').toContain(
